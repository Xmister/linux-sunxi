...
static DEFINE_PER_CPU(struct perf_event_attr *[CNTMAX], pevent_attr1);
extern DEFINE_PER_CPU(struct perf_event *, pevent_ebs1);
...
static int gator_events_perf_pmu_online(int** buffer)
{
	...
	for (cnt = 0; cnt < pmnc_counters; cnt++) {
        if (pmnc_count[cnt] > 0) {
            ev = per_cpu(pevent_ebs1, cpu); // special case for EBS
        } else {
...
static void gator_events_perf_pmu_online_dispatch(int cpu)
{
	...
	for (cnt = 0; cnt < pmnc_counters; cnt++) {
		if (per_cpu(pevent, cpu)[cnt] != NULL || per_cpu(pevent_attr1, cpu)[cnt] == 0)
			continue;
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(3, 1, 0)
		per_cpu(pevent, cpu)[cnt] = perf_event_create_kernel_counter(per_cpu(pevent_attr1, cpu)[cnt], cpu, 0, dummy_handler);
#else
		per_cpu(pevent, cpu)[cnt] = perf_event_create_kernel_counter(per_cpu(pevent_attr1, cpu)[cnt], cpu, 0, dummy_handler, 0);
#endif
...
static int gator_events_perf_pmu_start(void)
{
	...
	for_each_present_cpu(cpu) {
		for (cnt = 0; cnt < pmnc_counters; cnt++) {
			...
			per_cpu(perfPrevDelta, cpu)[cnt] = 0;
			per_cpu(pevent_attr1, cpu)[cnt] = kmalloc(size, GFP_KERNEL);
			if (!per_cpu(pevent_attr1, cpu)[cnt]) {
				gator_events_perf_pmu_stop();
				return -1;
			}

			memset(per_cpu(pevent_attr1, cpu)[cnt], 0, size);
			per_cpu(pevent_attr1, cpu)[cnt]->type = PERF_TYPE_RAW;
			per_cpu(pevent_attr1, cpu)[cnt]->size = size;
			per_cpu(pevent_attr1, cpu)[cnt]->config = pmnc_event[cnt];
			per_cpu(pevent_attr1, cpu)[cnt]->sample_period = 0;
			per_cpu(pevent_attr1, cpu)[cnt]->pinned = 1;

			// handle special case for ccnt
			if (cnt == ccnt) {
				per_cpu(pevent_attr1, cpu)[cnt]->type = PERF_TYPE_HARDWARE;
				per_cpu(pevent_attr1, cpu)[cnt]->config = PERF_COUNT_HW_CPU_CYCLES;
			}
		}
	}
...
static void gator_events_perf_pmu_stop(void)
{
	...
	for_each_present_cpu(cpu) {
		for (cnt = 0; cnt < pmnc_counters; cnt++) {
			if (per_cpu(pevent_attr1, cpu)[cnt]) {
				kfree(per_cpu(pevent_attr1, cpu)[cnt]);
				per_cpu(pevent_attr1, cpu)[cnt] = NULL;
			}
		}
	}
